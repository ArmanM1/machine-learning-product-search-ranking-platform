...
The public run contract keeps the validation-selected SageMaker training execution and the two held-out SageMaker Processing executions in distinct records. Each record publishes its own image digest, Git identity, hardware, region, runtime, estimate, actual cost when reconciled, and cost-evidence note. The training record remains bound to the frozen trial-selection/model-source SHA; the evaluation and release records bind the later release-orchestration SHA and its release-tagged evaluation image. Training runtime is taken from the checksummed selected `RunManifest`; evaluation runtime is the sum of the two Processing-job wall-clock intervals. These durations are never added together or presented under one hardware label.
...
Warm Lambda tests use candidate counts 10, 20, and 40 and offered concurrency 1, 4, and 8, with at least 200 requests after 10 explicit per-condition warmups for the release latency claim. Requests are emitted in fixed concurrency waves paced to a target 18 requests/second so offered load remains reproducible. Function URLs impose no configured fixed request-rate limit; Lambda reserved concurrency two is the capacity bound. The controlled cold request and all pre-benchmark observations are excluded from warm samples and percentiles. Report p50/p95/p99, successes, throttles/errors, sample count, Lambda memory, x86_64 architecture, `us-east-1`, reserved concurrency two, and model revision. Concurrency above two is expected to expose the configured Lambda bound; do not convert throttled offered load into a scale claim.
