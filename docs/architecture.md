...
Objects carry SHA-256 checksums. Final paths and release decisions are immutable; only the versioned current-release pointer and held-out counter intentionally advance. The counter remains at its non-expiring dedicated key. Its valid JSON is space-padded to `4096 + count` bytes so the write-only release role can recover the current sequence from exact-key `ListObjectVersions` metadata without gaining held-out read access. The repository-wide financial-operation concurrency group serializes advances; each successful write is bound to its returned checksum, ETag, encryption mode, and version ID before an immutable run reservation is published. Release never advances the live pointer: deployment does so only after production activation and verification. A failed held-out gate publishes a new release-specific evidence bundle whose active model remains the prior baseline. Raw and model artifacts stay private unless their terms have been reviewed for redistribution.

The release role is also intentionally write-only for `public/<release-id>/`. Those copies use create-only PUTs with explicit MD5 and SHA-256 values, SSE-S3, and validated write receipts. An existing public key fails closed rather than being treated as an identical retry without byte-read authority; a new release dispatch receives a new report-scoped prefix.
