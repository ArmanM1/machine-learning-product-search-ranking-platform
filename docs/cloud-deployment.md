...
13. `benchmark-serving.yml`: optional manual post-deployment evidence run over candidate counts 10/20/40 and offered concurrency 1/4/8, with 10 explicit warmups and 200 measured requests per condition. Fixed concurrency waves are paced to a target 18 requests/second for reproducible offered load; Function URLs add no configured fixed request-rate limit, and reserved Lambda concurrency two remains the measured capacity bound. Each successful rank response carries a numeric server-side serialization duration for the validated response's JSON-mode export and final byte rendering; the benchmark retains every raw value and recomputes separate percentiles rather than estimating serialization as end-to-end minus model time. It checksum-binds the separate controlled cold observation, excludes it and pre-benchmark observations from all warm percentiles, reports throttles above the reserved bound, and makes no throughput or scaling claim.
...
For every successful warm request, the strict report contract also requires a finite
non-negative in-process serialization sample and requires its p50/p95/p99/mean/minimum/maximum
summary to recompute exactly from the raw samples.
