...
Final public activation is still pending. Until a successful deployment artifact and independent browser check exist, `status.json` keeps `public_demo_deployed` false and the public URL and deployment checksum fields null. Candidate training, held-out evaluation, trained-model promotion, rollback proof, and teardown proof also remain false.
